...
def main():
    ...
    # Remove preprocessing old stuff
    # train_dataset = datasets.ImageFolder(traindir, transforms.Compose([transforms.Resize((224, 224)), transforms.ToTensor()]))
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset,
                                               ...
                                               pin_memory=True)
    val_loader = torch.utils.data.DataLoader(valid_dataset,#test_dataset,
                                             batch_size=args.batch_size,
                                             shuffle=False,
                                             num_workers=args.workers,
                                             pin_memory=True)
    ...
    if args.evaluate:
        validate(val_loader, classifier, criterion_val, args)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train_acc, train_los = train(train_loader, classifier, model_dis, criterion_train, optimizer, epoch, args)

        # evaluate on validation set
        val_acc, val_los = validate(val_loader, classifier, criterion_val, args)
        ...
        save_checkpoint({'state_dict': classifier.state_dict()}, is_best, args)
        end_time = time.time()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--af', '--adjust-freq', default=30, type=int, metavar='N',
                        help='adjust learning rate frequency')
    parser.add_argument('--momentum', default=0.9, type=float, metavar='M')
    parser.add_argument('--wd', '--weight-decay', default=5e-5, type=float, metavar='W', dest='weight_decay')   # 1e-4
